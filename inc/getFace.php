...
if(empty($username)) exit('Nie odnaleziono loginu!');

$filename = 'skintmp/' . md5($username) . '.png';
if(file_exists($filename) && time()-filemtime($filename) < 3600*24*3 && filesize($filename)) { 
...
imagecopyresized($av, $im, 0, 0, 8, 8, $size, $size, 8, 8); // Facexd
